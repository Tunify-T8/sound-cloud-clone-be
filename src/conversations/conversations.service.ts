...
export class ConversationsService {
  ...
  async deleteConversation(userId: string, conversationId: string) {
    ...
    return {
      message: `Conversation ${conversationId} deleted for user ${userId}`,
    };
  }
  ...
  async getMessages(
    ...
    const items = messages.map((message) => {
      ...
      if (type === 'TRACK_LIKE' && message.track) {
        ...
        (type === 'PLAYLIST' || type === 'ALBUM' || type === 'UPLOAD') &&
        message.collection
      ) {
  ...
  async markAs(userId: string, conversationId: string, flag: boolean) {
    ...
    return {
      message: `Conversation ${conversationId} marked as ${flag ? 'read' : 'unread'} for user ${userId}`,
    };
  }
  ...
  async archiveConversation(userId: string, conversationId: string) {
    ...
    return {
      message: `Conversation ${conversationId} archived for user ${userId}`,
    };
  }
  ...
  async unarchiveConversation(userId: string, conversationId: string) {
    ...
    return {
      message: `Conversation ${conversationId} unarchived for user ${userId}`,
    };
  }
